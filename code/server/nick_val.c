...
int check_nickname(char* nickname, char *errmsg) {
	...
	switch(check_res) {
		case 1: return NICK_TOO_SHORT;
		case 2: return NICK_TOO_LONG;
	}
	
	check_res = check_characters(nickname);
	switch(check_res) {
		case 1: return FIRST_CHAR_INV;
		case 2: return CONTAINS_INV_CHAR;
	}
	
	check_res = check_duplicity(nickname, NULL);
	switch(check_res) {
		case 1: return NICK_ALREADY_EXISTS;
	}
